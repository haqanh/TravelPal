      ...
      <div class="w-80vw p-10 my-10 mx-10">
        <div class="DashboardContents flex flex-col lg:flex-row justify-between lg:items-center -mx-10">
            ...
            <div class="relative flex items-center w-full h-full lg:w-96 group mt-4 lg:mt-0">
                ...
                <input type="text" v-model="searchInput" class="block w-full py-3 pl-10 pr-4 leading-normal rounded-xl focus:border-transparent focus:outline-none focus:ring-2 focus:ring-gray-800 ring-opacity-90 bg-gray-100 text-gray-[#3F3D3D] aa-input" placeholder="Search"/>
            </div>
        </div>
        <div class="flex justify-between items-center my-10 mb-10 min-w-full -mx-10">
        ...
        <AddTrip v-if="showAddTrip" @closetrip="resetAddTrip" @closetriponly="backToDashboard"/>
        <div class="TripCardWrapper grid grid-cols-4 min-w-full gap-4">
            <TripCard class="cursor-pointer" @contextmenu.prevent="openContextMenu($event, trip)" @click="viewTrip(trip)" v-for="trip in filteredTrips" :key="trip.id" :TripName="trip.Name" :Location="trip.Location" :TripStartDate="trip.Start_Date" :TripEndDate="trip.End_Date" :TripImage="trip.Photos[0]" :TripCost="trip.Cost"/>
        </div>
        <div class="flex justify-between items-center my-10 mb-10 min-w-full -mx-10">
        ...
        <AddGuide v-if="showAddGuide" @close="resetAddGuide"/>
        <div class="GuideCardWrapper grid grid-cols-4 min-w-full gap-4">
            <GuideCard class="cursor-pointer" @click="viewGuide(guide)" v-for="guide in filteredGuides" :key="guide.id" :GuideName="guide.Guide_Title" :Location="guide.Destination" :GuideImage="guide.Cover_Photo"/>
        </div>
    </div>
    </div> 
    <div v-if="contextMenuOpen" :style="{ top: `${contextMenuPosition.y}px`, left: `${contextMenuPosition.x}px` }" class="absolute bg-white border border-gray-200 rounded-md shadow-lg">
        <button @click="deleteTrip" class="block w-full text-left px-[0.7vw] py-[0.7vw] text-l text-gray-700 hover:bg-gray-100 flex justify-between items-center rounded-md shadow-lg">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 inline-block align-text-bottom mr-[0.7vw]">
                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
            </svg>
        <span class="mt-[0.3vw]">Delete</span>
        </button>
    </div>
    <DeleteTripModal v-if="showDeleteTripModal" @del="deleteDoc" @closeDel="closeDel" :name="selectedTrip.Name" />
</template>
...
import { getStorage, ref, deleteObject, listAll } from 'firebase/storage'
  ...
  export default {
    ...
    methods: {
        ...
        this.contextMenuPosition = { x: event.clientX + window.scrollX, y: event.clientY + window.scrollY};
